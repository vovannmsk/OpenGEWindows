...
namespace States
{
    public class StateGT44 : IState
    {
        ...
        public StateGT44(botWindow botwindow)   //, GotoTrade gototrade)
        {
        ...
        {
            return new StateGT45(botwindow);
        }
        ...
        {
            if (!botwindow.isHwnd()) return new StateGT50(botwindow);  //последнее состояние движка, чтобы движок сразу тормознулся
            if (server.isLogout())
            {
                return new StateGT43(botwindow);  //коннект и далее
            }
            else
            {
                return this;
