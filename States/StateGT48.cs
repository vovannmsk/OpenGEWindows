...
//        private Town town;
        ...
        public StateGT48(botWindow botwindow)   //, GotoTrade gototrade)
        {
...
//            return botwindow.isOpenMenuPet();     //сделать проверку, открыто ли окно с петом Alt+P
        ...
        {
            return new StateGT49(botwindow);
        }
        ...
        {
            if (!botwindow.isHwnd()) return new StateGT57(botwindow);  //последнее состояние движка, чтобы движок сразу тормознулся
            if (server.isLogout())
            {
