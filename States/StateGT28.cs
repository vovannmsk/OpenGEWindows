...
namespace States
{
    public class StateGT28 : IState
    {
        ...
        public StateGT28(botWindow botwindow)   //, GotoTrade gototrade)
        {
        ...
        {
            if (!botwindow.isHwnd()) return new StateGT01(botwindow);  //последнее состояние движка, чтобы движок сразу тормознулся
            if (server.isLogout())
            {
